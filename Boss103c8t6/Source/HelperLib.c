...
// кондер на первый фильтр задается в main()
// триггеры и прочее - не здесь, здесь только мгновенные значения от 0 до 1023.f
...
// сканирование одной ручки, преобразование в значение потов с зажатыми и не зажатыми кнопками с записью
void ScanPotsShadowProc(int p , int i){
	...
	if (delta >= 1) {
		...
		pots[i].trig[p] = kSet;
		// пересчет значений
		pots[i].val_old[p] = pots[i].val[p]  = pots[i].f2/1024.0;
